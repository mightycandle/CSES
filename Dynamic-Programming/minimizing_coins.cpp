...
const int inf=1e18,mod=1e9+7,maxn=1e8+1;
/**/
int fun(){
	read(int,n,x);
	vi a;bool fl=0;
	...
	if(fl)return 1;
	if(a.empty())return -1;
	set<int> st;each(x,a)st.insert(x);
	a.clear();revit(st)a.pb(*it);
	sort(all(a));n=a.size();
	vi dp(x+1,mod);each(i,a)dp[i]=1;
	int idx=0;
	for(int i=a[0]+1;i<=x;i++){
		idx+=(dp[i]==1);
		rep(j,idx+1)chmin(dp[i],1+dp[i-a[j]]);
	}
